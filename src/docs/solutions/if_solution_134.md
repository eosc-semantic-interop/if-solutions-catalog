---
title: "Make it a requirement that all PIDs must be valid URNs as defined by IANA."
desc: "Unknown"
type: Unknown
status: Unknown
problem_id:
  - T4
---

# Make it a requirement that all PIDs must be valid URNs as defined by IANA.

|Type|Status|
|----|------|
|Unknown|Unknown|

Problem ID(s) this solutions adresses:
- T4

## Description
Unknown

## Problems that CAN be solved with this IF solution


## Limitations of this IF solution


## Description of use case/scenario/case of study for this IF solution


## Link to documentation (URL)
[]()

## EOSC project/initiative, that is working on the solution


## Any other comment

